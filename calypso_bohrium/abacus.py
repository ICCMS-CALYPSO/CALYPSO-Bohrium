...
def abacus_task(pop, task_dir, N_INCAR, command, backward_files=["STRU_ION_D", "running_cell-relax.log"]):
   ...
   task = Task(
       ...
       forward_files=["STRU", "STRU.ori"] + [f"INPUT_{idx}" for idx in range(1, N_INCAR + 1)] + file_list,
...
def abacus_back(task_dir, pop):
    ...
    shutil.copyfile(os.path.join(task_dir, "OUT.ABACUS", "running_cell-relax.log"), "running_cell-relax.log_%d" % pop)
